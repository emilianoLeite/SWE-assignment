...
- **Handle `assigneeId=unassigned` as a special sentinel, not a valid ObjectId** — when the frontend sends `?assigneeId=unassigned`, `Types.ObjectId.isValid("unassigned")` returns false, silently skipping the filter. Fix: check for the literal string `"unassigned"` first and map it to `{ assigneeId: null }` in the Mongoose match stage (MongoDB `null` matches both `null` and missing fields). Only fall through to the ObjectId branch when the value is a valid hex ID.
